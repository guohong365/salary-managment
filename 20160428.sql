...
INSERT INTO `t_position_salary_items` VALUES ('0000000000','f4876dbc-b404-4f70-82ba-c898b2e35cfb','20160401',1),('0204030100','3d0d1b65-6b0d-4331-98cb-e2338adaac42','20160401',1),('0204030100','f4876dbc-b404-4f70-82ba-c898b2e35cfb','20160401',1);
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `v_salary_struct_detail` AS select `t`.`ID` AS `ID`,`t`.`NAME` AS `NAME`,`t`.`DESCRIPTION` AS `DESCRIPTION`,`t`.`TYPE` AS `TYPE`,`t`.`VALUE` AS `VALUE`,`t`.`VERSION_ID` AS `VERSION_ID`,`t`.`FUNC_ID` AS `FUNC_ID`,ifnull(`t1`.`POSITION_ID`,'0204030100') AS `POSITION_ID`,if(isnull(`t1`.`ENABLED`),FALSE,`t`.`ENABLED`) AS `ENABLED` from (`t_salary_item` `t` left join `t_position_salary_items` `t1` on(((`t1`.`SALARY_ITEM_ID` = `t`.`ID`) and (`t1`.`POSITION_ID` = '0204030100') and (`t`.`VERSION_ID` = `t1`.`VERSION_ID`)))) where ((`t`.`ENABLED` = TRUE) and ((`t1`.`POSITION_ID` = '0204030100') or (isnull(`t1`.`POSITION_ID`) and (`t`.`POSITION_ID` = '9999999999')))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2016-04-28 17:51:15
