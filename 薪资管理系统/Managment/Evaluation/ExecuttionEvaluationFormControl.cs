...
namespace SalarySystem.Managment.Evaluation
{
    public partial class ExecuttionEvaluationFormControl : BaseEditableControl
    {
        ...
        private void cellValueChanged(object sender, CellValueChangedEventArgs e)
        {
            ...
            if (e.Column == colPOSITION_ID)
            {
                ...
                if (row.IsNull("EVALUATION_FORM_ID")) //新行跳过
                    return;
                var formId = row.EVALUATION_FORM_ID;
                var fromRow = _evaluationForm.FindByID(formId);
            ...
            var formRow = _evaluationForm.FindByID((string) e.Value);
        ...
        {
            var row = gridViewExecutionForm.GetDataRow(e.RowHandle) as DataSetSalary.t_position_evaluation_formsRow;
            if (row == null) return;
            row.ENABLED = true;
